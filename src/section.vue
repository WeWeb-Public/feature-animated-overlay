    ...
    <div class="section-container">
        ...
        <div class="workflow-section-content">
            <ul class="workflow-list">
                <li class="workflow-wrapper" :class="{selected: isWorkFlowSelected(workflow)}" v-for="(workflow, index) in section.data.workflows" :key="workflow.uniqueId">
                    <!-- wwManager:start -->
                    <wwContextMenu ng-if="editMode" class="ww-orange-button" tag="div" :options="elemOptions" @remove="removeWorkflow(workflow)" @addBefore="addWorkflowBefore(index)" @addAfter="addWorkflowAfter(index)">
                        <wwOrangeButton></wwOrangeButton>
                    </wwContextMenu>
                    <!-- wwManager:end -->
                    <div class="workflow-item" :class="{selected: isWorkFlowSelected(workflow)}">
                        <div class="workflow-title">
                            <wwObject tag="span" :ww-object="workflow.title"></wwObject>
                        </div>
                        <div class="workflow-content">
                            <wwLayoutColumn tag="div" ww-default="ww-text" :ww-list="workflow.contentList" @ww-add="add(workflow.contentList, $event)" @ww-remove="remove(workflow.contentList, $event)">
                                <wwObject tag="div" ww-default="ww-text" v-for="item in workflow.contentList" :key="item.uniqueId" :ww-object="item"></wwObject>
                            </wwLayoutColumn>
                        </div>
                    </div>
                </li>
            ...
            <ul class="navigator" ref="navigator">
                <div class="navigator-selector" ref="navigatorSelector" v-bind:style="navigatorStyle"></div>
                <li v-for="(workflow, index) in section.data.workflows" :key="workflow.uniqueId">
                    <div class="navigator-item" @click="toggleWorkflow(workflow)">
                        <wwObject tag="span" :ww-object="workflow.navigatorText" class="navigator-text" :class="{selected:isWorkFlowSelected(workflow)}" ww-default="ww-text"></wwObject>
                    </div>
                </li>
            </ul>
        </div>
...
export default {
    ...
    computed: {
        ...
        navigatorStyle() {
            const offset = this.startingNavigatorPosition + this.section.data.workflows.indexOf(this.selectedWorkflow) * 120;
            return {
